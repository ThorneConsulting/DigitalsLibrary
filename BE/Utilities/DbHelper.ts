...
export class DbHelper {
    ...
    constructor() {
      this._logger = new Logger('DBHELPER');
    ...
    private async tryConnectionAsync(retryConnection: boolean) {
        ...
        if (retryConnection) {
            ...
            this._logger.log(message, LogLevelsEnum.DEFAULT);
    ...
    private async seedDbAsync() {
        ...
        try {
          ...
          if (RESULT) {
            this._logger.log('Data insert complete', LogLevelsEnum.DEFAULT);
          } else {
            this._logger.log('There was some problem inserting seed data', LogLevelsEnum.DEFAULT);
          }
        } catch (error) {
          this._logger.log(error, LogLevelsEnum.ERROR);
        }
      };
